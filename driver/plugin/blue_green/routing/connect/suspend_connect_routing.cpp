...
#include "suspend_connect_routing.h"
...
SQLRETURN SuspendConnectRouting::Connect(
    DBC* dbc,
    HostInfo info,
    ...
    std::shared_ptr<ConcurrentMap<std::string, BlueGreenStatus>> status_cache)
{
    ...
    const std::chrono::milliseconds timeout = MapUtils::GetMillisecondsValue(conn_attr, KEY_BG_CONNECT_TIMEOUT_MS, DEFAULT_CONNECT_TIMEOUT_MS);
    ...
    if (cached_status.GetCurrentPhase().GetPhase() == BlueGreenPhase::IN_PROGRESS) {
        ...
        error_msg += " ms. Try connecting again later.";
        LOG(ERROR) << error_msg;
