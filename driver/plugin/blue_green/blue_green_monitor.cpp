...
BlueGreenMonitor::BlueGreenMonitor(
    ...
    const HostInfo& initial_host_info,
    ...
    blue_green_id_{ std::move(blue_green_id) },
    initial_host_info_{ initial_host_info },
    conn_attr_{ std::move(conn_attr) },
    check_interval_map_{ std::move(check_interval_map) },
    on_status_change_function_{ std::move(on_status_change_function) }
{
...
void BlueGreenMonitor::StartMonitoring() {
    bool expected = false;
    const std::lock_guard<std::mutex> thread_lock(monitor_mutex_);
...
void BlueGreenMonitor::ResetCollectedData() {
    {
        const std::lock_guard<std::mutex> init_ip_lock(initial_ip_host_map_mutex_);
        this->initial_ip_host_map_.clear();
    }
    {
        const std::lock_guard<std::mutex> init_topology_lock(initial_topology_mutex_);
        this->initial_topology_.clear();
    }
    {
        const std::lock_guard<std::mutex> host_name_lock(host_names_mutex_);
        this->host_names_.clear();
    }
}

void BlueGreenMonitor::Stop() {
    const std::unique_lock<std::mutex> lock_guard(finish_mutex_);
...
void BlueGreenMonitor::Run() {
    while (class_running_.load()) {
        const std::lock_guard<std::mutex> lock_guard(hdbc_mutex_);
        if (!class_running_.load()) {
            // Exit early on monitor reset
            break;
        }

        const BlueGreenPhase old_phase = this->current_phase_;
        this->OpenConnection();
        this->CollectStatus();
        ...
        const BlueGreenIntervalRate rate = this->in_panic_mode_ ? BlueGreenIntervalRate::HIGH : this->interval_rate_.load();
        const std::chrono::milliseconds delay_ms = check_interval_map_.contains(rate) ?
            check_interval_map_.at(rate) : DEFAULT_INTERVAL_MS;
        this->Delay(delay_ms);
    }
...
void BlueGreenMonitor::Delay(std::chrono::milliseconds delay_ms) {
    ...
    const std::chrono::steady_clock::time_point end_time = start_time + delay_ms;
    const std::chrono::milliseconds minimum_delay = delay_ms < MINIMUM_DELAY_MS ? delay_ms : MINIMUM_DELAY_MS;

    const BlueGreenIntervalRate current_interval_rate = this->interval_rate_;
    const bool current_panic = this->in_panic_mode_;
...
void BlueGreenMonitor::CollectHostIp() {
...

    {
        ...
        if (!this->host_names_.empty()) {
            for (const std::string& host : host_names_) {
                this->current_ip_host_map_.try_emplace(host, BlueGreenMonitor::GetIpAddress(host));
            }
        }
    }
...
void BlueGreenMonitor::UpdateIpAddressFlags() {
    ...
    const std::lock_guard<std::mutex> init_ip_lock(initial_ip_host_map_mutex_);
    const auto initial_end = initial_ip_host_map_.end();
    const auto current_end = current_ip_host_map_.end();
    ...
    if (!this->collect_ip_) {
        ...
        this->all_start_topology_ip_changed_ = !this->initial_topology_.empty()
                ...
                [&](const HostInfo& info)
            {
                ...
                const auto itr_initial = initial_ip_host_map_.find(host);
                const auto itr_current = current_ip_host_map_.find(host);
    ...
    if (!this->collect_topology_) {
        ...
        for (const HostInfo& host : this->initial_topology_) {
            initial_topology_nodes.insert(host.GetHost());
        }

        const std::vector<HostInfo> current_topology(this->current_topology_);
...
std::optional<std::string> BlueGreenMonitor::GetIpAddress(std::string host) {
    ...
    const int status = getaddrinfo(host.c_str(), nullptr, &hints, &servinfo);
    if (status != 0) {
...
void BlueGreenMonitor::CollectTopology() {
    ...
    const std::vector<HostInfo> current_topology = this->host_list_provider_->ForceRefresh(false, std::chrono::seconds(5));
    if (!current_topology.empty()) {
        ...
        if (this->collect_topology_) {
            ...
            {
                const std::lock_guard<std::mutex> host_name_lock(host_names_mutex_);
                for (const HostInfo& host : current_topology) {
...
void BlueGreenMonitor::CollectStatus() {
    ...
    if (!SQL_SUCCEEDED(res.fn_result)) {
        ...
        const std::string error_msg = (stmt != SQL_NULL_HANDLE) ? this->odbc_helper_->GetStmtErrorMessage(stmt) : "";
    ...
    static constexpr SQLUSMALLINT COL_ENDPOINT = 2;
    static constexpr SQLUSMALLINT COL_PORT = 3;
    static constexpr SQLUSMALLINT COL_ROLE = 4;
    static constexpr SQLUSMALLINT COL_STATUS = 5;
    static constexpr SQLUSMALLINT COL_VERSION = 6;
    ...
    this->odbc_helper_->BindCol(&stmt, COL_ENDPOINT, SQL_C_TCHAR, &endpoint, sizeof(endpoint), &len);
    this->odbc_helper_->BindCol(&stmt, COL_PORT, SQL_INTEGER, &port, sizeof(port), &len);
    this->odbc_helper_->BindCol(&stmt, COL_ROLE, SQL_C_TCHAR, &role, sizeof(role), &len);
    this->odbc_helper_->BindCol(&stmt, COL_STATUS, SQL_C_TCHAR, &status, sizeof(status), &len);
    this->odbc_helper_->BindCol(&stmt, COL_VERSION, SQL_C_TCHAR, &version, sizeof(version), &len);
    res = this->odbc_helper_->Fetch(&stmt);
    while (SQL_SUCCEEDED(res.fn_result)) {
        const std::string version_str = AS_UTF8_CSTR(version);
        const std::string endpoint_str = AS_UTF8_CSTR(endpoint);
        const std::string role_str = AS_UTF8_CSTR(role);
        const std::string status_str = AS_UTF8_CSTR(status);
        const int local_port = static_cast<int>(port);
        const BlueGreenRole local_role = BlueGreenRole::ParseRole(role_str, version_str);
        const BlueGreenPhase local_phase = BlueGreenPhase::ParsePhase(status_str, version_str);

        if (this->current_role_ == local_role) {
            status_entries.push_back({.version = version_str, .endpoint = endpoint_str, .port = local_port, .phase = local_phase, .role = local_role});
        }
        res = this->odbc_helper_->Fetch(&stmt);
    }
    this->odbc_helper_->BaseFreeStmt(&stmt);

    const auto itr_end = status_entries.end();
    ...
    if (itr != itr_end) {
        ...
        const std::string search = ".cluster-";
        const size_t begin_idx = converted_endpoint.find(search);
    ...
    if (collect_topology_) {
        for (const StatusInfo& info : status_entries) {
    ...
    if (!this->connecting_host_info_correct_ && itr != itr_end) {
        ...
        const std::string endpoint_ip = BlueGreenMonitor::GetIpAddress(itr->endpoint).value_or("");
        const std::string connected_ip = this->connecting_ip_;
...
void BlueGreenMonitor::OpenConnection() {
    ...
    const HostInfo connecting_host_info = this->connecting_host_info_;
    const std::string connecting_ip = this->connecting_ip_;
    ...
    const SQLRETURN rc = this->plugin_head_->Connect(local_hdbc, nullptr, nullptr, 0, nullptr, SQL_DRIVER_NOPROMPT);

    if (SQL_SUCCEEDED(rc)) {
        this->connecting_ip_ = BlueGreenMonitor::GetIpAddress(connecting_host_info.GetHost()).value_or("");
...
void BlueGreenMonitor::NotifyChanges() {
    const std::lock_guard<std::mutex> lock_guard(sleep_mutex_);
...
void BlueGreenMonitor::InitHostListProvider() {
    ...
    const std::string custom_cluster_id = this->blue_green_id_ + "-" + this->current_role_.ToString() + "-" + BG_CLUSTER_ID;
