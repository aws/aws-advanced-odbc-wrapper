...
int32_t BlueGreenInterimStatus::GetHashCode() {
    int32_t result = GetValueHash(1, this->phase_.ToString());
    result = GetValueHash(result, this->version_);
    result = GetValueHash(result, std::to_string(this->port_));
    result = GetValueHash(result, this->all_start_topology_ip_changed_ ? "true" : "false");
    result = GetValueHash(result, this->all_start_topology_endpoints_removed_ ? "true" : "false");
    result = GetValueHash(result, this->all_topology_changed_ ? "true" : "false");

    for (const std::string& host_name : this->host_names_) {
        result = GetValueHash(result, host_name);
    }
    for (const HostInfo& info : this->initial_topology_) {
        result = GetValueHash(result, info.GetHostPortPair() + std::to_string(info.GetHostRole()));
    }
    for (const HostInfo& info : this->current_topology_) {
        result = GetValueHash(result, info.GetHostPortPair() + std::to_string(info.GetHostRole()));
    }
    for (const auto& [key, value] : this->initial_ip_host_map_) {
        result = GetValueHash(result, key + value.value_or(""));
    }
...
int32_t BlueGreenInterimStatus::GetValueHash(int current_hash, std::string value) {
