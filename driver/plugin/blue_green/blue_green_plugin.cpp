...
std::recursive_mutex BlueGreenPlugin::provider_lock_;
    ...
    SQLUSMALLINT   DriverCompletion)
{
    ...
    // A status exist, get / increment provider if not already assigned
    this->InitProvider();
    lock_guard.unlock();
    ...
    if (host_role.GetRole() == BlueGreenRole::UNKNOWN) {
        LOG(INFO) << "[FALLBACK-B] Default connection, unexpected role: UNKNOWN, host: " << conn_host
                  << ", phase: " << this->blue_green_status_.GetCurrentPhase().ToString();
        return InitConnection(ConnectionHandle, WindowHandle, OutConnectionString, BufferLength, StringLengthPtr, DriverCompletion);
    }
    ...
    if (connect_routes.empty()) {
        LOG(INFO) << "[FALLBACK-C] Default connection, no routes found for: " << conn_host
                  << ", phase: " << this->blue_green_status_.GetCurrentPhase().ToString()
                  << ", role: " << host_role.ToString();
        return InitConnection(ConnectionHandle, WindowHandle, OutConnectionString, BufferLength, StringLengthPtr, DriverCompletion);
    }
    ...
    if (route_itr == connect_routes.end()) {
        LOG(INFO) << "[FALLBACK-D] Default connection, no routes matched for role: " << host_role.ToString()
                  << ", host: " << conn_host
                  << ", phase: " << this->blue_green_status_.GetCurrentPhase().ToString()
                  << ", total_routes: " << connect_routes.size();
        return InitConnection(ConnectionHandle, WindowHandle, OutConnectionString, BufferLength, StringLengthPtr, DriverCompletion);
    }
    ...
    try {
        ...
        while (route_itr != connect_routes.end() && !SQL_SUCCEEDED(rc)) {
            loop_iteration++;
            const std::string current_route = (*route_itr)->ToString();
            LOG(INFO) << "[ROUTE-LOOP] iteration=" << loop_iteration << ", route=" << current_route;
            ...
            if (!SQL_SUCCEEDED(rc)) {
                ...
                if (refetched_phase.GetPhase() == BlueGreenPhase::UNKNOWN) {
                    ...
                    LOG(WARNING) << "[FALLBACK-E] Statuses reset mid-loop, falling back to direct. role: " << host_role.ToString() << ", host: " << conn_host;
                ...
                const bool route_found = route_itr != connect_routes.end();
                const bool same_as_last = route_found && (*route_itr)->ToString() == last_failed_route;
                LOG(INFO) << "[ROUTE-LOOP] re-fetch: phase=" << refetched_phase.ToString()
                          << ", routes_count=" << connect_routes.size()
