...
std::vector<HostInfo> ClusterTopologyMonitor::OpenAnyConnGetHosts() {
    ...
    if (!main_hdbc_) {
        ...
        if (!main_hdbc_) {
            ...
            const std::string writer_id = query_helper_->GetWriterId(local_dbc);
            if (!writer_id.empty()) {
    ...
    try {
        while (!main_monitor_->node_threads_stop_.load()) {
            if (GetNodeId(hdbc_, main_monitor_->dialect_).empty()) {
                ...
                // Get Writer ID
                const std::string writer_id = main_monitor_->query_helper_->GetWriterId(hdbc_);
...
void ClusterTopologyMonitor::NodeMonitoringThread::ReaderThreadFetchTopology() {
    ...
    // Query for hosts
    const std::vector<HostInfo> hosts = main_monitor_->query_helper_->QueryTopology(local_hdbc);
