...
std::vector<HostInfo> ClusterTopologyMonitor::FetchTopologyUpdateCache(const SQLHDBC hdbc) {
    ...
    if (query_helper_->GetNodeId(hdbc).empty()) {
...
RDS_STR ClusterTopologyMonitor::ConnForHost(const std::string& new_host) {
    ...
    return ConnectionStringHelper::BuildMinimumConnectionString(conn_map);
}
...
std::vector<HostInfo> ClusterTopologyMonitor::OpenAnyConnGetHosts() {
    ...
    auto* conn_cstr = AS_SQLTCHAR(ConnectionStringHelper::BuildMinimumConnectionString(connection_attributes_).c_str());
    if (!main_hdbc_) {
        SQLHDBC local_hdbc;
...
bool ClusterTopologyMonitor::GetPossibleWriterConn() {
    ...
    if (SQL_NULL_HDBC != local_hdbc && !query_helper_->GetNodeId(local_hdbc).empty() && local_hostinfo.IsHostUp()) {
...
void ClusterTopologyMonitor::NodeMonitoringThread::Run() {
    ...
    DBC* dbc = (DBC*) hdbc_;
    ConnectionStringHelper::ParseConnectionString(updated_conn_str, dbc->conn_attr);

    try {
        bool should_stop = main_monitor_->node_threads_stop_.load();
        while (!should_stop) {
            if (main_monitor_->query_helper_->GetNodeId(hdbc_).empty()) {
...
void ClusterTopologyMonitor::NodeMonitoringThread::ReaderThreadFetchTopology() {
    ...
    // Check connection
    if (main_monitor_->query_helper_->GetNodeId(local_hdbc).empty()) {
        return;
    };
    // Query for hosts
    std::vector<HostInfo> hosts;
