    ...
    const std::shared_ptr<OdbcHelper> &odbc_helper)
{
    ...
    const RdsLibResult res = odbc_helper->AllocStmt(dbc->wrapped_dbc, dbc->env->driver_lib_loader, stmt);
    ...
    const SQLLEN ind_router_endpoint_value = 0;

    SQLTCHAR load_value[LOAD_LENGTH] = {};
    const SQLLEN ind_load_value = 0;

    odbc_helper->SQLBindCol(dbc->env->driver_lib_loader, stmt, 1, SQL_C_TCHAR, &router_endpoint_value, sizeof(router_endpoint_value), ind_router_endpoint_value);
    odbc_helper->SQLBindCol(dbc->env->driver_lib_loader, stmt, 2, SQL_C_TCHAR, &load_value, sizeof(load_value), ind_load_value);

    odbc_helper->SQLExecDirect(dbc->env->driver_lib_loader, stmt, limitless_router_endpoint_query);

    std::vector<HostInfo> limitless_routers;

    while (SQL_SUCCEEDED((odbc_helper->SQLFetch(dbc->env->driver_lib_loader, stmt)).fn_result)) {
    ...
    odbc_helper->FreeStmt(dbc->env->driver_lib_loader, stmt);
