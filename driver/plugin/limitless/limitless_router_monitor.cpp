...
LimitlessRouterMonitor::LimitlessRouterMonitor(BasePlugin* plugin_head, const std::shared_ptr<DialectLimitless>& dialect, const std::shared_ptr<OdbcHelper> &odbc_helper_) {
...
void LimitlessRouterMonitor::Open(
    ...
    ENV* env;
    odbc_helper_->AllocEnv(henv, env, lib_loader_);
    odbc_helper_->SQLSetEnvAttr(lib_loader_, env, SQL_ATTR_ODBC_VERSION, reinterpret_cast<SQLPOINTER>(SQL_OV_ODBC3), 0);
    ...
    if (block_and_query_immediately) {
        ...
        const SQLRETURN rc = plugin_head_->Connect(
            ...
            SQL_DRIVER_NOPROMPT);
        if (SQL_SUCCEEDED(rc)) {
            const std::vector<HostInfo> new_limitless_routers = LimitlessQueryHelper::QueryForLimitlessRouters(local_hdbc, host_port, dialect_, odbc_helper_);
...
void LimitlessRouterMonitor::Run(SQLHENV henv, SQLHDBC conn, const std::map<std::string, std::string>& conn_attr, int host_port) {
    ...
    do {
        ...
        if (conn == SQL_NULL_HANDLE || GetNodeId(conn, dialect_).empty()) {
            if (conn) {
                odbc_helper_->DisconnectAndFree(conn, lib_loader_);
                conn = SQL_NULL_HANDLE;
            }

            dbc = odbc_helper_->AllocDbc(henv, conn);
        ...
        const std::vector<HostInfo> new_limitless_routers = LimitlessQueryHelper::QueryForLimitlessRouters(conn, host_port, dialect_, odbc_helper_);
