...
SimpleReadWriteSplittingPlugin::SimpleReadWriteSplittingPlugin(DBC *dbc, std::shared_ptr<BasePlugin> next_plugin) : AbstractReadWriteSplittingPlugin(dbc, next_plugin) {
...
SQLRETURN SimpleReadWriteSplittingPlugin::Connect(
    ...
    if (RdsUtils::IsRdsWriterClusterDns(host) || this->verify_initial_conn_type_ == WRITER) {
        ret = GetVerifiedConnection(host, WRITER, ConnectionHandle, WindowHandle, OutConnectionString, BufferLength, StringLengthPtr, DriverCompletion, conn);
    } else if (RdsUtils::IsRdsReaderClusterDns(host) || this->verify_initial_conn_type_ == READER) {
        ret = GetVerifiedConnection(host, READER, ConnectionHandle, WindowHandle, OutConnectionString, BufferLength, StringLengthPtr, DriverCompletion, conn);
    }
...
void SimpleReadWriteSplittingPlugin::SetInitialConnectionHostInfo(SQLHDBC conn, std::string host) {
    if (const std::shared_ptr<PluginService> service = plugin_service_.lock()) {
        const HOST_ROLE role = service->GetHostListProvider()->GetConnectionRole(conn);
        const HostInfo host_info = CreateHostInfo(host, role);
        service->SetInitialHostInfo(host_info);
    }
}

SQLRETURN SimpleReadWriteSplittingPlugin::GetVerifiedConnection(
    std::string host,
    HOST_ROLE role,
    SQLHDBC ConnectionHandle,
    SQLHWND WindowHandle,
    ...
    DBC*& conn)
{
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        ...
        if (const std::shared_ptr<PluginService> service = plugin_service_.lock();
            conn == SQL_NULL_HDBC || !SQL_SUCCEEDED(ret) || service->GetHostListProvider()->GetConnectionRole(conn) != role) {
...
bool SimpleReadWriteSplittingPlugin::ShouldUpdateReaderConnection(const HostInfo &current_host) {
    ...
    if (const std::shared_ptr<PluginService> service = plugin_service_.lock()) {
        return current_connection_ != nullptr &&
            ...
            (!this->verify_new_conns_ || service->GetHostListProvider()->GetConnectionRole(current_connection_) == READER);
    }
    return current_connection_ != nullptr &&
            current_host.GetHostRole() == READER &&
            cached_reader_conn != nullptr &&
            cached_reader_conn->wrapped_dbc != current_connection_ &&
            !this->verify_new_conns_;
}
...
bool SimpleReadWriteSplittingPlugin::ShouldUpdateWriterConnection(const HostInfo &current_host) {
    if (const std::shared_ptr<PluginService> service = plugin_service_.lock()) {
        return current_connection_ != nullptr &&
            ...
            (!this->verify_new_conns_ || service->GetHostListProvider()->GetConnectionRole(current_connection_) == WRITER);
    }
    return current_connection_ != nullptr &&
            current_host.GetHostRole() == WRITER &&
            writer_connection_ != nullptr &&
