...
SQLRETURN AbstractReadWriteSplittingPlugin::Execute(SQLHSTMT StatementHandle, SQLTCHAR *StatementText, SQLINTEGER TextLength) {
    ...
    HostInfo curr_host;
    if (const std::shared_ptr<PluginService> service = plugin_service_.lock()) {
        read_only = SqlQueryAnalyzer::DoesSetReadOnly(query, service->GetDialect());
        curr_host = service->GetCurrentHostInfo();
    }

    const STMT* stmt = static_cast<STMT*>(StatementHandle);
    const DBC* dbc = stmt->dbc;

    if (!this->plugin_head_) {
    ...
    std::string state = this->odbc_helper_->GetSqlStateAndLogMessage(nullptr);
...
void AbstractReadWriteSplittingPlugin::UpdateInternalConnectionInfo() {
    ...
    if (const std::shared_ptr<PluginService> service = plugin_service_.lock()) {
        current_host = service->GetCurrentHostInfo();
    }
...
SQLRETURN AbstractReadWriteSplittingPlugin::SwitchConnectionIfRequired(bool read_only, const HostInfo &current_host) {
    if (current_connection_ != nullptr && odbc_helper_->IsClosed(dbc_)) {
    ...
    if (read_only) {
        if (current_host.GetHostRole() == READER) {
            ret = SQL_SUCCESS;
        } else if (dbc_->transaction_status != TRANSACTION_OPEN) {
            ret = SwitchToReaderConnection(current_host);
        }

        if (!SQL_SUCCEEDED(ret)) {
            if (odbc_helper_->IsClosed(dbc_)) {
...
SQLRETURN AbstractReadWriteSplittingPlugin::SwitchToWriterConnection(const HostInfo &current_host) {
    if (current_host.GetHostRole() == WRITER && !odbc_helper_->IsClosed(dbc_)) {
        // Already connected to the writer.
        return SQL_SUCCESS;
    }

    SQLRETURN ret = SQL_ERROR;
    if (!this->writer_connection_ || odbc_helper_->IsClosed(this->writer_connection_)) {
...
void AbstractReadWriteSplittingPlugin::SwitchCurrentConnectionTo(DBC* new_conn, const HostInfo &new_host) {
    ...
    if (const std::shared_ptr<PluginService> service = plugin_service_.lock()) {
        service->SetCurrentHostInfo(new_host);
    }
...
SQLRETURN AbstractReadWriteSplittingPlugin::SwitchToReaderConnection(const HostInfo &current_host) {
    if (current_host.GetHostRole() == READER && !odbc_helper_->IsClosed(dbc_)) {
    ...
    DBC* reader_conn = GetCurrentReaderConn();
    SQLRETURN ret = SQL_ERROR;
    if (reader_conn == nullptr || odbc_helper_->IsClosed(reader_conn)) {
