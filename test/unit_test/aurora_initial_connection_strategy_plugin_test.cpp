...
protected:
    ...
    HostInfo* writer_host = new HostInfo("instance-1.XYZ.us-east-2.rds.amazonaws.com", 1234, UP, WRITER, HostInfo::DEFAULT_WEIGHT );
    HostInfo* reader_host_a = new HostInfo("instance-2.XYZ.us-east-2.rds.amazonaws.com", 1234, UP, READER, 3 );
    HostInfo* reader_host_b = new HostInfo("instance-3.XYZ.us-east-2.rds.amazonaws.com", 1234, UP, READER, 2 );
    HostInfo* reader_host_c = new HostInfo("instance-4.XYZ.us-east-2.rds.amazonaws.com", 1234, UP, READER, 1 );
...
TEST_F(AuroraInitialConnectionStrategyPluginTest, Connect_Success_Writer_Cannot_Find_Writer_Retry) {
    ...
    std::vector<HostInfo> empty_host_vector;
    EXPECT_CALL(*mock_plugin_service,
        GetHosts())
    // .Times(testing::Exactly(2))
    .WillOnce(testing::Return(empty_host_vector))
    .WillOnce(testing::Return(empty_host_vector))
...
TEST_F(AuroraInitialConnectionStrategyPluginTest, Connect_Success_Writer_Network_Error_Retry) {
    EXPECT_CALL(
        *mock_base_plugin,
        Connect(testing::_, testing::_, testing::_, testing::_, testing::_, testing::_))
    .Times(testing::Exactly(3))
    .WillOnce(testing::Return(SQL_ERROR)) // try #1
    .WillOnce(testing::Return(SQL_ERROR)) // try #2
    .WillOnce(testing::Return(SQL_SUCCESS));; // try #3
