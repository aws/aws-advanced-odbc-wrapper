...
TEST_F(AdfsAuthPluginTest, Connect_Success) {
    ...
    AdfsAuthPlugin plugin(dbc_, mock_base_plugin_, mock_saml_util_, mock_auth_provider_);
...
TEST_F(AdfsAuthPluginTest, Connect_Success_CacheExpire) {
    ...
    AdfsAuthPlugin plugin(dbc_, mock_base_plugin_, mock_saml_util_, mock_auth_provider_, mock_dialect_, mock_odbc_helper_);
    SQLRETURN ret = plugin.Connect(dbc_, nullptr, nullptr, 0, 0, SQL_DRIVER_NOPROMPT);
    EXPECT_EQ(SQL_SUCCESS, ret);
