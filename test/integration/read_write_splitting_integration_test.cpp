...
TEST_F(ReadWriteSplittingIntegrationTest, WriterConnectSetReadOnly) {
    ...
    ODBC_HELPER::Query(dbc, stmt_set_read_only_true);
    ...
    ODBC_HELPER::Query(dbc, stmt_set_read_only_false);
...
TEST_F(ReadWriteSplittingIntegrationTest, ReaderClusterConnectSetReadOnly) {
    std::string reader_cluster_conn_str = ConnectionStringBuilder(test_dsn, cluster_ro_url, test_port)
            .withUID(test_uid)
            .withPWD(test_pwd)
            .withDatabase(test_db)
            .withEnableRwSplitting(true)
...
TEST_F(ReadWriteSplittingIntegrationTest, ReadOnlyFalseInTx) {
    ...
    EXPECT_EQ(SQL_ERROR, ODBC_HELPER::ExecuteQuery(handle, stmt_set_read_only_false));
...
TEST_F(ReadWriteSplittingIntegrationTest, ReadOnlyTrueInTx) {
    if (test_dialect != "AURORA_MYSQL") {
        GTEST_SKIP() << "ReadOnlyTrueInTx is only supported for AURORA_MYSQL dialect.";
    }
...
TEST_F(ReadWriteSplittingIntegrationTest, ReadOnlyFalseAutocommitFalse) {
    ...
    EXPECT_EQ(SQL_ERROR, ODBC_HELPER::ExecuteQuery(hstmt, stmt_set_read_only_false));
...
// TODO: failover tests can be flaky, uncomment when stable
// TEST_F(ReadWriteSplittingIntegrationTest, WriterFailoverReadOnly) {
//     std::string failover_conn_str = ConnectionStringBuilder(conn_str)
//         .withEnableClusterFailover(true)
//         .getString();
//     SQLRETURN rc = ODBC_HELPER::DriverConnect(dbc, failover_conn_str);
//     EXPECT_EQ(SQL_SUCCESS, rc);
//
//     std::string current_connection_id = QueryInstanceId(dbc);
//
