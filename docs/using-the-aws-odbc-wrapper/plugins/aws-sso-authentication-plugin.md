...
| SSO Region                | `SSO_REGION`                | The region of the IAM Identity Center instance, used only for the IAM Identity Center sign-in. Falls back to `REGION`, then the region derived from `SERVER`, then `us-east-1`.                                       | derived       | `us-east-1`                                    |
| Region                    | `REGION`                    | The region of the database, used to sign the RDS IAM authentication token. Falls back to the region derived from `SERVER`, then `us-east-1`.                                                                          | derived       | `us-west-1`                                    |
...
> [!NOTE]\
> `SSO_REGION` and `REGION` are independent, so the IAM Identity Center instance does not have to be in the same region as the database. `SSO_REGION` is only used to reach the IAM Identity Center endpoints during sign-in, while `REGION` is the region the RDS IAM authentication token is signed for. When the two regions differ, set `REGION` (or use a database endpoint the region can be derived from) so the token is signed for the database's region. A token signed for the wrong region is rejected by the database.
