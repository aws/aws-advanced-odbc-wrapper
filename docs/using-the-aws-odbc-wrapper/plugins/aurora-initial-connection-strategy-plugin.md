...
|---------------------------------------------|----------------------------------------|:--------:|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------|---------------|
| Reader Host Selector Strategy               | `HOST_SELECTOR_STRATEGY`               |    No    | The strategy that will be used to select a new reader host when opening a new connection. <br><br> Currently supported strategies are: `RANDOM_HOST`, `ROUND_ROBIN`, `HIGHEST_WEIGHT`.         | `RANDOM`      | `ROUND_ROBIN` |
| Verify Initial Connection Type              | `VERIFY_INITIAL_CONNECTION_TYPE`       |    No    | Override the connection type by specifying either `WRITER` or `READER`. If not specified, the default behaviour will be applied, and the connection type will be determined by the server URL. | nil           | `WRITER`      |
