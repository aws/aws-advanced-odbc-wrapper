...
| Verify Initial Connection Type              | `VERIFY_INITIAL_CONNECTION_TYPE`       |    No    | If the wrapper cannot determine if the server url is either writer or reader cluster URL, this property can be used as a fallback by specfying `WRITER` or `READER`.                   | nil           | `WRITER`      |
