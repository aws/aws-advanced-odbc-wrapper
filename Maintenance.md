...
|-------------------|--------------------------------------------------------------------------------------|
| September 8, 2026 | [Release 1.3.1](https://github.com/aws/aws-advanced-odbc-wrapper/releases/tag/1.3.1) |
| July 30, 2026     | [Release 1.3.0](https://github.com/aws/aws-advanced-odbc-wrapper/releases/tag/1.3.0) |
...
For minor version releases, `aws-advanced-odbc-wrapper` uses a "release-train" model. Approximately every four weeks we
release a new minor version which includes all the new features and fixes that are ready to go.
Having a set release schedule makes sure `aws-advanced-odbc-wrapper` is released in a predictable way and prevents a
backlog of unreleased changes.

In contrast, `aws-advanced-odbc-wrapper` releases new major versions only when there are a critical mass of
breaking changes (e.g. changes that are incompatible with existing APIs). This tends to happen if we need to
change the way the driver is currently working. Fortunately, the ODBC API is fairly mature and has not changed, however
...
For `aws-advanced-odbc-wrapper` new features and active development always takes place against the newest version.
The `aws-advanced-odbc-wrapper` project follows the semantic versioning specification for assigning version numbers
...
the major version number (e.g., increment from `aws-advanced-odbc-wrapper` 1.1.1 to `aws-advanced-odbc-wrapper` 2.0.0).
...
|---------------|----------------------|-------------|-----------------|--------------------------|------------------------|
| 1             | 1.3.1                | Current     | Nov 20, 2025    | N/A                      | N/A                    |
