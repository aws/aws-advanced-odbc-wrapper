...
int main() {
    ...
    SQLCHAR* conn_str = reinterpret_cast<SQLCHAR*>(
        ...
        "RDS_AUTH_TYPE=SECRETS_MANAGER;"
        "SECRET_ID=arn:aws:secretsmanager:us-west-2:123412341234:secret:my-secret-name-123456;"
        // Optional
        "SECRET_REGION=us-west-2;"      // If providing a full ARN, this value is not required
        "SECRET_ENDPOINT=localhost;"    // If using custom Secrets Manager provider
    ...
    // Connect
    rc = SQLDriverConnect(dbc, nullptr, conn_str, SQL_NTS, conn_out, MAX_NAME_LEN, &len, SQL_DRIVER_NOPROMPT);
